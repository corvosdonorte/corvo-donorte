...
function cls(t){return t.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");}

function coroa(tipo){
  return `<span class="coroa ${tipo}">👑</span>`;
}
...
async function vitoria(i){
  if(admin) updateDoc(doc(db,"ranking",grupo[i].id),{pontos:grupo[i].pontos+1}).then(carregarRanking);
}
async function derrota(i){
  if(admin && grupo[i].pontos>0)
    updateDoc(doc(db,"ranking",grupo[i].id),{pontos:grupo[i].pontos-1}).then(carregarRanking);
}
