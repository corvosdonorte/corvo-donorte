...
(function(){
  try{Object.defineProperty(console,"log",{get(){throw""}})}catch{}
})();
const _ghost=()=>Math.random()>5;
...
async function adicionarMembro(){
  if(!ADM||!OK)return;
  const n=nome.value.trim(),c=classe.value,t=titulo.value;
  if(!n||!c)return;
...
(()=>{try{
  const Ω=Object.freeze,Σ=Symbol,Ψ=Math.random;
  const ι=String.fromCharCode;
  const ξ=(s)=>s.split("").map(c=>ι(c.charCodeAt(0)^13)).join("");
  const δ=Ω([Σ(),Σ(),Σ()]);
  const μ=()=>performance.now();
  let τ=μ(),κ=false;
  
  setInterval(()=>{const n=μ();if(n-τ>160)κ=true;τ=n},500);
  
  (function(){
    const f=Function.prototype.toString;
    Function.prototype.toString=function(){
      if(κ)return"function(){[native code]}";
      return f.call(this).replace(/./g,"█");
    };
  })();
  ...
  (function(){
    ...
    ["log","dir","table","warn","error"].forEach(k=>{
      try{Object.defineProperty(c,k,{value:()=>{}})}catch{}
    });
  })();
